...
struct PomodoroPersistenceTests {
    ...
    func invalidSettingsUseCompleteDefaultsWithoutChangingCountdown(record: String) throws {
        ...
        #expect(restored.countdown.remaining == 1_200)
    ...
    func pomodoroStartupRestoresThenPausesUnrelatedCountdownWithoutStartupSounds() {
        ...
        session.countdownStore.save(CountdownSession(
        ...
        #expect(restored.pomodoro.status == .ready)
        #expect(restored.countdown.isPaused)
        #expect(restored.countdown.remaining == 899)
        #expect(restored.countdown.completionCount == 0)
        #expect(session.sounds == 0)
        session.now += 60
        restored.update()
        restored.selectMode(.countdown)
        #expect(restored.countdown.remaining == 899)
        #expect(restored.countdown.isPaused)
        restored.toggleCountdownRunning()
        session.now += 60
        restored.update()
        #expect(restored.countdown.remaining == 839)
        session.now += 839
        restored.update()
        #expect(restored.countdown.completionCount == 1)
    ...
    @Test(arguments: CountdownRecord.allCases, ["missing", "invalid", "Countdown", "Pomodoro"])
    func countdownRestorationKeepsItsRulesBeforeTheHiddenPauseGate(record: CountdownRecord, selectedMode: String) throws {
        ...
        let restored = session.makeTimer()
        #expect(restored.mode == (selectedMode == "Pomodoro" ? .pomodoro : .countdown))
        #expect(restored.pomodoro.status == .ready)
        #expect(restored.countdown.completionCount == 0)
        switch record {
        case .active:
            #expect(restored.countdown.status == (selectedMode == "Pomodoro" ? .prepared : .active))
            #expect(restored.countdown.remaining == 899)
        ...
        case .staleActive, .stalePrepared, .expired, .invalid:
            #expect(restored.countdown.status == .empty)
            #expect(restored.countdown.remaining == 0)
            #expect(session.sounds == 0)
        }
    }
    ...
    func unreadableSettingsFallBackWithoutReplacingCountdown() throws {
        ...
        defer { session.removeState() }
        let timer = session.makeTimer()
        timer.adjustCountdownDuration(by: 900)
        timer.selectMode(.pomodoro)
        timer.adjustPomodoroDuration(.focus, by: -300)
        timer.save()
    ...
    private final class Session {
        ...
        var countdownStore: CountdownStateStore {
            CountdownStateStore(environment: ["XDG_STATE_HOME": directory.path])
        }

        var settingsURL: URL {
            directory.appendingPathComponent("countdown/timer-settings.json")
        }

        func makeTimer(configuration: CountdownConfiguration = CountdownConfiguration(alarmNotificationURL: nil)) -> TimerController {
            TimerController(
