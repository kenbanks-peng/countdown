...
struct CountdownModeTests {
    ...
    func selectionAndRestartKeepOneModeAndSharedPause(outgoing: CountdownMode, incoming: CountdownMode) throws {
        ...
        let features = try JSONDecoder().decode([String: Bool].self, from: Data(contentsOf:
            session.store.stateDirectory.appendingPathComponent("features.json")))
        #expect(features["clock_enabled"] == nil)
        #expect(features["popup_enabled"] == false)
    }

    @Test(arguments: CountdownMode.allCases, CountdownMode.allCases)
    func selectionSettlesTimeoutUnderOutgoingMode(outgoing: CountdownMode, incoming: CountdownMode) {
        for autoset in [false, true] {
            for alarm in [false, true] {
                let session = Session()
                defer { session.close() }
                let controller = session.makeController(autoset: autoset, alarm: alarm)
                ...
                #expect(controller.timer.remaining == (outgoing.usesTimer && autoset ? 3_600 : 0))
                #expect(!controller.countdown.isPaused)
                controller.update()
                #expect(controller.timer.completionCount == (outgoing.usesTimer ? 1 : 0))
            }
    ...
    func timerControlsAndTimeoutEnablementWorkInBothModes(mode: CountdownMode, timeout: Bool) {
        ...
        controller.timer.setCurrentTimeoutEnabled(timeout)
        ...
        #expect(controller.timer.isCurrentTimeoutEnabled == timeout)
    ...
    private final class Session {
        ...
        func makeController(autoset: Bool = false, alarm: Bool = true) -> CountdownController {
            CountdownController(
                stateStore: store, configuration: CountdownConfiguration(alarmNotificationURL: nil),
                featureState: CountdownFeatureState(autosetEnabled: autoset, popupEnabled: false, alarmEnabled: alarm),
