...
struct ReminderTests {
    ...
    func unsupportedIntervalsUseDefault(value: Int) {
        #expect(CountdownConfiguration(alarmNotificationURL: nil, reminderTime: value).reminderTime == 5)
    }

    @Test(arguments: [5, 10, 15, 20, 60, 120])
    func supportedIntervalsAreUnchanged(value: Int) {
        #expect(CountdownConfiguration(alarmNotificationURL: nil, reminderTime: value).reminderTime == value)
    }

    @Test(arguments: [0, 1, 59])
    func firstReminderRoundsUpThenKeepsItsClockSchedule(seconds: Int) {
        ...
        let features = CountdownFeatures(
            configuration: CountdownConfiguration(alarmNotificationURL: nil, reminderTime: 15),
        ...
        #expect(features.reminderIntervalCount == 0)
        advance(1) // 08:25
        #expect(features.reminderIntervalCount == 1)
        advance(899)
        #expect(features.reminderIntervalCount == 1)
        advance(1) // 08:40
        #expect(features.reminderIntervalCount == 2)
        advance(2 * 900 + 30) // Late update at 09:10:30 emits once.
        #expect(features.reminderIntervalCount == 3)
        advance(869)
        #expect(features.reminderIntervalCount == 3)
        advance(1) // 09:25, not 09:25:30.
        #expect(features.reminderIntervalCount == 4)
    }
    ...
    func resumeSkipsPausedRemindersAndKeepsOriginalClockTimes(mode: CountdownMode) {
        ...
        controller.toggleRunning()
        #expect(controller.features.reminderIntervalCount == 0)
        now += 14 * 60 - 1
        controller.update()
        #expect(controller.features.reminderIntervalCount == 0)
        now += 1 // 08:55: original schedule, not a new interval from resume.
        controller.update()
        #expect(controller.features.reminderIntervalCount == 1)
    }
}
